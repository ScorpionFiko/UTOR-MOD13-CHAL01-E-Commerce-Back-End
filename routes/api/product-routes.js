...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    if (!productData) {
      res.status(404).json({ message: `No data found for product with id: ${req.params.id}!` });
      return;
    }
    res.status(200).json(productData);
  } catch (err) {
...
// create new product
router.post('/', (req, res) => {
  const { product_name, category_id, price, stock, tagIds } = req.body;
  if (!product_name || !category_id || !price || !stock || !tagIds ) {
